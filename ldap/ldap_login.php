<?php
	...
	$ldap_dn = "uid=".$ldap_username.",cn=users,cn=accounts,dc=sig,dc=corp";
	...
	if (@ldap_bind($ldap_con, $ldap_dn, $ldap_password)){
		...
		$sql_get = mysqli_query ($bd, "SELECT * FROM users WHERE ldap_mapping = '$email'");
        ...
        if ($num_row ==0) {
        	...
        	$insert = mysqli_query ($bd,"INSERT INTO users (tipe_login,username,fullname,role,employee_number,profile,phone_number,address,date_created,date_password_created,date_password_expiry,latest_ip_addr,ldap_mapping) VALUES ('LDAP','$ldap_username','$fullname','1','0','1','$phone','unknown',NOW(),NOW(),NOW() + interval '$exp' day,'$ip','$email')");

        	$sql_get = mysqli_query ($bd, "SELECT * FROM users WHERE username = '$ldap_username'");
            ...
            //exit()
			$_SESSION['SESS_ID'] = $id;
	        $_SESSION['SESS_USERNAME'] = $ldap_username;
	        $_SESSION['SESS_PASSWORD'] = $ldap_password;
	        $_SESSION['SESS_ROLE'] = $ri;
	        $last_login = mysqli_query($bd, "UPDATE users set last_login = NOW() WHERE id= '$id'");
	        $_SESSION['IP'] = $ip;
			session_write_close();
	        header("location: ../home.php?ap=ldap");
	        exit();
        ...
        session_write_close();
        header("location: ldap_index.php?msg=$regmsg");
        exit();
	}
?>