<?php
	...
	$ldap_con = ldap_connect("ipa3.sig.corp"); //server link
	...
	if (@ldap_bind($ldap_con, $ldap_dn, $ldap_password)){
        ...
        if ($num_row ==0) {
        	...
        	$insert = mysqli_query ($bd,"UPDATE users set ldap_mapping = '$email',tipe_login = '$type,LDAP' WHERE id = $id");
        	...
        	$regmsg = "<div class='alert alert-warning alert-dismissible fade show' role='alert' id='regsuccess'> Akun LDAP sudah terdaftar.";
        	session_write_close();
        	header("location: ldap_sync.php?msg=$regmsg");
        	exit();
        }
